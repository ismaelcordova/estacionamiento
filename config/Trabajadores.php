<?php
/**
 * Consultas relacionadas con la tabla `trabajadores` y `sin_vigencia`.
 ...
 */
function obtenerTrabajadores(string $tipo): array {
    $conn = getConexion();
 ...
 */
function buscarTrabajador(string $numero, string $tipo): ?array {
    $conn = getConexion();
 ...
 */
function agregarTrabajador(string $nombre, string $numero, string $marca, string $placa, string $tipo): ?array {
    $conn = getConexion();
 ...
 * Evita que el nombre de columna llegue directo desde el navegador.
 */
const CAMPOS_EDITABLES = ['Folio', 'Nombre_completo', 'Numero_plaza', 'Marca_automovil', 'Placa'];
 ...
 */
function actualizarCampoTrabajador(int $id, string $campo, string $valor): bool {
    if (!in_array($campo, CAMPOS_EDITABLES, true)) {
        return false;
    }
 ...
 */
function establecerVigencia(int $id, bool $vigente): bool {
    $conn = getConexion();
